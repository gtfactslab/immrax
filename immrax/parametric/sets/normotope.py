...
class Normotope (Parametope) :
    ...
    """
    def __init__ (self, ox:ArrayLike,  alpha:ArrayLike, y:Float, alpha_inv:Array|None=None) :
    ...
    def norm_ball_iover (cls, n:int) -> Interval :
        ...
        raise NotImplementedError("Subclasses must implement the norm_ball_iover method.")

    def iover (self) -> Interval :
        """An interval overapproximation of the normotope, defaults to interval analysis"""
        return interval(self.alpha_inv) @ (self.norm_ball_iover(self.ox.shape[0]) * self.y) + self.ox

    def plot_projection (self, ax:Axes, xi:int=0, yi:int=1, rescale:bool=False, **kwargs) -> None :
        """Plot the projection of the normotope onto the xi-yi plane."""
        raise NotImplementedError("Subclasses must implement the plot_projection method.")
...
class NormotopeEmbedding (ParametricEmbedding) :
    ...
    def _initialize (self, nt0:Normotope, *, no_gsc=False) -> ArrayLike :
        ...
        if not no_gsc :
            ix0 = nt0.iover()
            M = self.Mf(0., ix0, centers=((jnp.zeros(1), nt0.ox),))[0][1]
    ...
    def _dynamics (self, t, state, U, *, perm=None, adjoint=True) : 
        ...
        MM = self.Mf (t, nt.iover(), centers=((jnp.zeros(1), nt.ox),), permutations=perm)[0]
        Mx = MM[1]
...
class L1Normotope (Normotope) :
    ...
    def from_interval (cls, *args) -> 'L1Normotope' :
        cent, pert = i2centpert(interval(*args))
        return L1Normotope(cent, jnp.diag(1/pert), 1.)
...
class L2Normotope (Normotope) :
    ...
    def plot_projection (self, ax:Axes, xi:int=0, yi:int=1, rescale:bool=False, **kwargs) -> None :
        # self.to_polytope().plot_projection(ax, xi, yi, rescale, **kwargs)
        Ellipsoid(self.ox, self.alpha/self.y, jnp.array([0., 1.])).plot_projection(ax, xi, yi, rescale, **kwargs)
